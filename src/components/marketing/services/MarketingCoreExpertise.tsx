        ...
        <FadeInStagger>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 border border-border-light rounded-sm overflow-hidden">
            {expertise.map((item, index) => (
              <FadeItem 
                key={index} 
                className={`p-10 space-y-8 transition-all duration-500 hover:bg-neutral/50 group
                  ${index !== expertise.length - 1 ? 'border-b md:border-b-0 md:border-r border-border-light' : ''}`}
              >
                <item.icon className={`w-8 h-8 ${theme.text.brand} transition-transform duration-500 group-hover:scale-110`} strokeWidth={1.5} />
                ...
                <ul className="space-y-4">
                  {item.points.map((point, pIdx) => (
                    <li key={pIdx} className="flex items-center gap-3">
                      {/* Using tertiary (gold) for the architectural bullets */}

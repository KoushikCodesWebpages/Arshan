...
export default function HRPlanSelectionModal({ 
  ...
  return (
    <div className="fixed inset-0 z-10000 flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-500">
      ...
      <div className={`bg-white rounded-brand w-full max-w-7xl overflow-hidden shadow-[0_50px_100px_-20px_rgba(0,0,0,0.4)] animate-in fade-in zoom-in-95 slide-in-from-bottom-12 duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] relative`}>
        ...
        <div className="px-10 py-12">
          ...
          <div className="text-center mb-10 animate-in fade-in slide-in-from-top-4 duration-1000 delay-200">
            <span className="uppercase tracking-[0.2em] text-[11px] font-bold text-tertiary">
              HR Service Tiers
            </span>
            <h2 className={`text-4xl font-bold mt-1 ${theme.text.brand}`}>
              Change your Plan
            </h2>
            <p className={`text-sm mt-2 max-w-xl mx-auto ${theme.text.muted}`}>
          ...
          {hrPlans.map((plan, index) => {
            ...
            return (
              <div 
                ...
                className={`
                  relative p-10 rounded-lg flex flex-col border transition-all duration-700 group
                  animate-in fade-in zoom-in-95 slide-in-from-bottom-8 fill-mode-both
                  ${plan.featured 
                    ? `${theme.brand.primary} text-white border-transparent shadow-2xl lg:scale-[1.04] z-10 hover:scale-[1.07]` 
              ...
              >
                ...
                {plan.featured && (
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-tertiary text-white text-[10px] font-bold uppercase tracking-widest px-6 py-2 rounded-2xl shadow-lg z-20 whitespace-nowrap">
                    Most Selected
                  </div>
                )}

                {/* HEADER SECTION: Price below subtitle */}
                <div className="flex flex-col gap-5 mb-8 relative z-10">
                  <div>
                    <h4 className="text-xl font-bold tracking-tight group-hover:text-tertiary transition-colors duration-300">
                      {plan.name}
                    </h4>
                    <p className={`text-[13px] leading-relaxed mt-1 italic font-medium ${plan.featured ? "text-white/70" : "text-secondary"}`}>
                      {plan.subtitle}
                    </p>
                  </div>
                  ...
                  <div className="flex items-baseline gap-1.5">
                    ...
                    <span className="text-[11px] font-bold opacity-60 tracking-wider uppercase">/ month</span>
                  </div>
                </div>
                ...
                <ul className="space-y-4 mb-10 flex-1 relative z-10">
                  {plan.features.map((feat, i) => (
                    <li 
                      key={feat} 
                      className="flex items-start gap-3 text-[14px] leading-snug opacity-90 group-hover:translate-x-1 transition-transform duration-300"
                ...
                <button 
                  ...
                  disabled={isCurrent}
                  className={`
                    relative z-10 w-full py-5 rounded-2xl font-bold text-[12px] uppercase tracking-widest transition-all duration-300 active:scale-95
                    ${isCurrent 
        ...
        <div className="bg-neutral border-t border-border-light px-10 py-4 flex justify-between items-center text-[11px] text-secondary font-bold tracking-widest uppercase animate-in slide-in-from-bottom-full duration-1000 delay-700 fill-mode-both">

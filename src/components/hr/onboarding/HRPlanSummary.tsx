...
export default function HRPlanSummary({ selectedPlan, setSelectedPlan }: HRPlanSummaryProps) {
  ...
  // 1. Sync with LocalStorage on Mount
  useEffect(() => {
  ...
  // 2. Wrapper function to save change to localStorage
  ...
  return (
    <>
      {/* 20% Smaller Layout: max-w-[1120px] */}
      <section className="mb-16 max-w-280 mx-auto">
        <div className="flex flex-col lg:flex-row items-start lg:items-center gap-10 lg:gap-12">
          ...
          <div 
            ...
            className={`lg:w-2/3 w-full ${theme.brand.primary} rounded-brand p-8 text-white flex flex-col md:flex-row justify-between items-center shadow-xl relative group cursor-pointer overflow-hidden transition-all duration-500 hover:shadow-2xl hover:-translate-y-0.5`}
          >
            ...
            <div className="relative z-10 text-center md:text-left">
              ...
              <h4 key={selectedPlan.name} className="text-xl md:text-2xl font-bold mt-1 tracking-tight animate-in fade-in slide-in-from-left-2 duration-500">
            ...
            <div className="relative z-10 mt-6 md:mt-0 text-center md:text-right flex flex-col items-center md:items-end">
              <div className="text-2xl font-bold tracking-tighter">
                €{selectedPlan.price}
                <span className="text-[10px] opacity-50 font-bold uppercase tracking-widest ml-2">/ Month</span>
              </div>
      ...
      <HRPlanSelectionModal 
        ...
        onSelectPlan={handlePlanChange} // Uses the new handler that saves to storage
      />
    </>
  );
}